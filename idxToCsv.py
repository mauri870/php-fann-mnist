...
def splitPerc(l, perc):
    ...
    return np.split(l, splits[:-1]*len(l))
...
def makeFANNFileSplit( images, labels, filename, test_size=70 ):
    if len( images['images'] ) != len( labels['labels'] ):
        raise Exception( 'makeFANNFileSplit: input/label length mismatch' )
    ninstances = abs((len( images[ 'images' ] ) * test_size) / 100 )
    with file( filename, 'w' ) as f:
        ninputs = images['nrows'] * images['ncolumns']
        noutputs = labels['nclasses']
        classMap = labels['classMap']
        f.write( "%d %d %d\n" % ( ninstances, ninputs, noutputs ) )
        for i in range( ninstances ):
            x = reduce( lambda a,b: a+b, images['images'][i] )
            y = convertOrdinalToBinary( classMap[ labels['labels'][i] ], noutputs )
            f.write( '%s\n' % ( reduce( lambda a,b: a+b, [ '%f ' % ( val / 255. ) for val in x ] ) ) )
    ...
    with file( 'val.fann', 'w' ) as f:
        ninputs = images['nrows'] * images['ncolumns']
        noutputs = labels['nclasses']
        classMap = labels['classMap']
        f.write( "%d %d %d\n" % ( (len( images['images']) - ninstances ), ninputs, noutputs ) )
        for i in range(ninstances, len(images['images'])):
            x = reduce( lambda a,b: a+b, images['images'][i] )
            y = convertOrdinalToBinary( classMap[ labels['labels'][i] ], noutputs )
            f.write( '%s\n' % ( reduce( lambda a,b: a+b, [ '%f ' % ( val / 255. ) for val in x ] ) ) )
            f.write( '%s\n' % ( reduce( lambda a,b: a+b, [ '%f ' % ( val ) for val in y ] ) ) )
...
if __name__ == "__main__":
    ...
    elif options.outputFormat == "FANN_SPLIT":
        makeFANNFileSplit( images, labels, options.outputFile )
